...
import { StatusType, VIZEMO_STAGES, LEGACY_STAGES } from '../types';
import { getStatusColor, getVizeClass, getStatusClass } from '../utils/helpers';

const STATUS_OPTS: StatusType[] = [...VIZEMO_STAGES, ...LEGACY_STAGES];
...
export default function VizemoPipeline() {
    ...
    // Vizemo Pipeline: Exclude Leodessa pipeline explicitly or legacy data + vizemo targeted data.
    const vizemoCustomers = useMemo(
        () => customers.filter(c => c.pipelineType !== 'leodessa'),
        [customers]
    );

    const consultants = useMemo(() => {
        const set = new Set<string>();
        vizemoCustomers.forEach(c => { if (c.danisman) set.add(c.danisman); });
        return Array.from(set).sort();
    }, [vizemoCustomers]);

    const statusCounts = useMemo(() => {
        const m: Partial<Record<StatusType, number>> = {};
        vizemoCustomers.forEach(c => { m[c.durum] = (m[c.durum] ?? 0) + 1; });
        return m;
    }, [vizemoCustomers]);

    const danismanCounts = useMemo(() => {
        const m: Record<string, number> = {};
        vizemoCustomers.forEach(c => {
    ...
    }, [vizemoCustomers, filterDurum, filterDanisman, search, sortCol, sortDir]);
    ...
    const col = (c: typeof sortCol, label: string) => (
        <th
            ...
            style={{
                cursor: 'pointer',
                color: sortCol === c ? 'var(--primary)' : undefined
            }}
        >
            ...
            <div style={{
                ...
                <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
                        ...
                        <h1 style={{ fontSize: '28px', color: 'var(--primary)' }}>Vizemo Pipeline</h1>
                    </div>
                    <div style={{ color: 'var(--text-muted)', fontSize: '13px', fontFamily: "'DM Sans', sans-serif" }}>
                        Şuan Vizemo veritabanında {vizemoCustomers.length} müşteri var. Temsilciler: {consultants.join(' & ') || 'Yok'}
                    </div>
                </div>

                {/* Stat cards - Horizontally scrollable for many statuses */}
                    ...
                    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                        {consultants.map(d => (
                            <button
                                ...
                                style={{
                                    padding: '6px 14px',
                                    background: filterDanisman === d ? `rgba(59, 130, 246, 0.22)` : 'var(--bg-elevated)',
                                    border: filterDanisman === d ? `1px solid rgba(59, 130, 246, 0.88)` : '1px solid var(--border-subtle)',
                                    borderRadius: 20,
                                    cursor: 'pointer',
                                    fontSize: '12px',
                                    color: filterDanisman === d ? 'var(--primary)' : 'var(--text-secondary)',
